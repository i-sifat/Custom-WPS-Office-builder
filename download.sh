...
for cmd in wget dpkg-deb sed mkdir rm; do
    if ! command -v "$cmd" >/dev/null 2>&1; then
        fatal "Missing required tool: $cmd\nInstall with: sudo apt-get install -y wget dpkg-dev coreutils"
    fi
done
...
if [[ "$MODE" == "download" ]] || [[ "$MODE" == "both" ]]; then
    ...
    local file_size
    file_size=$(stat -c%s "$DEB_NAME" 2>/dev/null || stat -f%z "$DEB_NAME" 2>/dev/null)
...
if [[ "$MODE" == "extract" ]] || [[ "$MODE" == "both" ]]; then
    ...
    # Check if .deb exists
    if [[ ! -f "$DEB_NAME" ]]; then
        fatal "No .deb file found: $DEB_NAME\nRun with --download first, or place the .deb in this folder."
    fi
